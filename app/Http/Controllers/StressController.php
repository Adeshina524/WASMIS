...
use App\Mail\SevereStressAlert;
use App\Models\User;
...
class StressController extends Controller
{
    ...
    public function submit(Request $request)
    {
        ...
        for ($i = 1; $i <= 7; $i++) $rules["c$i"] = 'required|integer|min:1|max:5';
        ...
        // ── SECTION C: Academic Stressors (7 items) ──
        ...
        // ── STRESS LEVEL CLASSIFICATION (locked thresholds — Table 3.1) ──
        if ($stress_score <= 47) {
            $stress_level = 'Mild';
        } elseif ($stress_score <= 57) {
            $stress_level = 'Moderate';
        } elseif ($stress_score <= 67) {
            $stress_level = 'High';
        } else {
            $stress_level = 'Severe';
        }

        // Check if this student is CROSSING INTO Severe (wasn't severe last time, or this is their first assessment)
        $previousRecord = StressRecord::where('user_id', auth()->id())->latest()->first();
        $wasSeverePreviously = $previousRecord && $previousRecord->stress_level === 'Severe';
        $isCrossingIntoSevere = ($stress_level === 'Severe') && !$wasSeverePreviously;

        $record = StressRecord::create([
            'user_id'               => auth()->id(),
        ...
        if ($isCrossingIntoSevere) {
            $this->notifyCounselors($record);
        }

        return redirect()
            ->route('student.dashboard')
            ->with('result', "Stress Level: $stress_level (Score: $stress_score / 107)");
    }
    ...
    private function notifyCounselors(StressRecord $record): void
    {
        $counselors = \App\Models\User::where('role', 'counselor')->get();

        foreach ($counselors as $counselor) {
            \Illuminate\Support\Facades\Mail::to($counselor->email)
                ->send(new \App\Mail\SevereStressAlert($record));
        }
    }
}